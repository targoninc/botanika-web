...
export class ChatTemplates {
    ...
    static chatHistory() {
        ...
        const dedupHistory = compute(h => {
            h = h.sort((a, b) => a.createdAt - b.createdAt);
        ...
        return create("div")
            ...
            .children(
                ...
                create("div")
                    ...
                    .children(
                        ChatTemplates.toolCalls(message as AssistantMessage),
                ...
                when("finished" in message && message.finished, ChatTemplates.messageActions(message)),
    ...
    static messageActions(message: ChatMessage) {
        ...
        return create("div")
            ...
            .children(
                ("hasAudio" in message && message.hasAudio) ? button({
                ...
                when(message.type === "assistant", create("span")
                    .classes("text-small")
                    .text(message.type === "assistant" ? `Generated by ${message.provider}/${message.model}` : "")
                    .build())
            ).build();
    }
    ...
    private static toolCalls(message: AssistantMessage) {
        const expanded = signal(false);
        const sources = message.toolInvocations?.filter(ti => ti.state === "result").flatMap(ti => "references" in ti.result && ti.result.references ? ti.result.references : []) ?? [];
    ...
    private static reasoning(message: ChatMessage) {
        const expanded = signal(false);
        const hasReasoning = "reasoning" in message && (message.reasoning?.length ?? 0 > 0);
        const icon = compute((e): string => e ? "keyboard_arrow_down" : "keyboard_arrow_right", expanded);
                ...
                when(expanded, create("div")
                    ...
                    .children(
                        create("div")
                            ...
                            .html(parseMarkdown((("reasoning" in message ? message.reasoning : []) ?? []).reduce((acc, r) => {
                                if (r.type === "text") {
                                    acc += r.text;
                                } else {
